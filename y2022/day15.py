...
def get_intervals(
    sensors: list[Coord], beacons: list[Coord], y: int
) -> list[Interval | None]:
    return [
        interval(sensor, manhattan(sensor, beacon), y=y)
...
def no_beacons(sensors, beacons, y):
    ...
    counted_beacons = sum(
        in_intervals(intervals, bx) for bx, by in set(beacons) if by == y
    )
...
sensors = [
    (int(a[2:-1]), int(b[2:-1])) for line in lines for a, b in [line.split(" ")[2:4]]
]
beacons = [
    (int(a[2:-1]), int(b[2:-1])) for line in lines for a, b in [line.split(" ")[8:]]
]
