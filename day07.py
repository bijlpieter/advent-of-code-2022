...
class Directory:
    ...
    def compute_size(self):
        self.size = (
            sum(file.size for file in self.files.values())
            + sum(directory.compute_size() for directory in self.directories.values())
        )

        return self.size

    def get_sizes(self):
        return [self.size] + [size for directory in self.directories.values() for size in directory.get_sizes()]
