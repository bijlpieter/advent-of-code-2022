...
north = df.iloc[:, :].cummax(axis=0).diff(axis=0).ne(0).iloc[:, :]
south = df.iloc[::-1, :].cummax(axis=0).diff(axis=0).ne(0).iloc[::-1, :]

west = df.iloc[:, :].cummax(axis=1).diff(axis=1).ne(0).iloc[:, :]
east = df.iloc[:, ::-1].cummax(axis=1).diff(axis=1).ne(0).iloc[:, ::-1]
